...
const Home = () => {
    ...
    const slides = [
        { data: weekData, title: "Turtle Mode 🐢 (Weekly Performance)", color: "#8884d8" },
        { data: monthData, title: "Rabbit Mode 🐇 (Monthly Performance)", color: "#82ca9d" },
        { data: yearData, title: "Eagle Mode 🦅 (Yearly Performance)", color: "#ffc658" }
    ];
            ...
            <div className='absolute top-0 w-full h-full flex flex-col justify-center text-white'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto p-4'>
                    ...
                    <div className='space-y-5 flex flex-col justify-center'>
                        <p>Welcome To</p>
                        <h1 className='font-bold text-5xl md:text-7xl drop-shadow-lg'>Trading Buddy</h1>
                        <p className='max-w-[600px] drop-shadow-2xl py-2 text-m'>
                            A one-stop shop for everything and anything related to stocks. Click below to sign up and gain access
                        ...
                        <div>
                            <button className='bg-white text-black font-sans hover:bg-gray-200 transition-all'>Sign Up Now</button>
                        </div>
                    </div>

                    {/* Right Column - Stock Graph */}
                    <div className='bg-gray-900/80 rounded-lg p-4 shadow-lg transform translate-x-12 md:mt-15'>
                        <h2 className='text-xl font-bold mb-2 text-center'>{slides[currentSlide].title}</h2>
                        <h3 className='text-center mb-4'>Boeing Stock</h3>
                        <div className='h-64'>
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={slides[currentSlide].data}margin={{ top: 5, right: 26, left: 0, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" stroke="#333" />
                                    <XAxis dataKey="date" stroke="#fff" tickMargin={10} interval={0} />
                                    <YAxis stroke="#fff" />
                                    <Tooltip 
