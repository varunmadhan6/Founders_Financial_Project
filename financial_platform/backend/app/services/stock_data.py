...
class HistoricalStockService:
    ...
    @staticmethod
    def insert_day_range_data_with_movement(symbols):
        """
        For each date between 2025-04-25 and 2025-05-02 and each provided stock symbol:
        - Fetch the historical row for that date from yFinance
        - Compute 52-week high/low up to that day
        - Compare to the previous day's row from DB
        - Insert that day into stock_data and update market_pulse
        """
        start_date = datetime.strptime('2025-04-25', '%Y-%m-%d').date()
        end_date = datetime.strptime('2025-05-02', '%Y-%m-%d').date()

        day_list = pd.date_range(start=start_date, end=end_date, freq='B').date  # Business days only
        ...
        for current_date in day_list:
            ...
            for symbol in symbols:
                try:
                    ...
                    data_full = ticker.history(start=current_date - timedelta(days=365), end=current_date + timedelta(days=1))
                    if data_full.empty:
                        continue

                    data_full['52wk_high'] = data_full['High'].rolling(window=252, min_periods=1).max()
                    data_full['52wk_low'] = data_full['Low'].rolling(window=252, min_periods=1).min()

                    row = data_full.loc[data_full.index.date == current_date]
                    if row.empty:
                        continue

                    valid_data_found = True
                    current_row = row.iloc[-1]
                    closing_price = current_row['Close']
                    current_52wk_high = current_row['52wk_high']
                    current_52wk_low = current_row['52wk_low']

                    # Fetch previous row from DB
                    conn = get_db_connection()
                    cur = conn.cursor()
                    cur.execute("""
                    ...
                    is_advanced = is_declined = is_unchanged = is_new_high = is_new_low = False
                    ...
                    """, (symbol, current_date, closing_price, current_52wk_high, current_52wk_low))
                    conn.commit()
                    cur.close()
                    conn.close()
                    ...
                    print(f"Error on {current_date} for {symbol}: {e}")
                    continue
            ...
            if valid_data_found:
                ...
                """, (current_date, total_new_highs, total_new_lows, total_advanced, total_declined, total_unchanged, ad_spread))
                conn.commit()
                cur.close()
                conn.close()
